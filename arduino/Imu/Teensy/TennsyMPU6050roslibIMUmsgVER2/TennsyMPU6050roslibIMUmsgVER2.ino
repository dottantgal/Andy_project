...
* It uses fifo buffer and normal methods to extrapolate orientation, acceleration and velocity
* 
* PINOUT:
*         MPU6050       TEENSY LC
*         Vcc           3v3
...
void setup()
{
  ...
  nh.loginfo(accelgyro.testConnection() ? "[low level imu] MPU6050 connection successful" : "[low level imu] MPU6050 connection failed");
...
void loop()
{
  ...
  if ((mpuIntStatus & 0x10) || fifoCount == 1024)
  {
    ...
    accelgyro.resetFIFO();
    nh.loginfo("[low level imu] FIFO overflow!");
...
//
//    imu_msg.orientation.w = w/16384.0;
//    imu_msg.orientation.x = x/16384.0;
//    imu_msg.orientation.y = y/16384.0;
//    imu_msg.orientation.z = z/16384.0;

    //Acceleration values
    imu_msg.linear_acceleration.x = get_ax * (double) ACC_FACTOR * G_FACTOR;
    imu_msg.linear_acceleration.y = get_ay * (double) ACC_FACTOR * G_FACTOR;
    imu_msg.linear_acceleration.z = get_az * (double) ACC_FACTOR * G_FACTOR;

    //Velocity values
    imu_msg.angular_velocity.x = get_gx * (double) GYRO_FACTOR * DEG_TO_RAD;// rad / sec
    imu_msg.angular_velocity.y = get_gy * (double) GYRO_FACTOR * DEG_TO_RAD;
    imu_msg.angular_velocity.z = get_gz * (double) GYRO_FACTOR * DEG_TO_RAD;
